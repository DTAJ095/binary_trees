 ...
 */
avl_t *binary_tree_rotation(avl_t **tree, avl_t *last, int flag_right)
{
	avl_t *new_parent;
	...
	if (flag_right)
	{
		...
		new_parent = last->parent;
		last->parent = NULL;
		last = binary_tree_rotate_right(last);
		last->parent = new_parent;
		if (last->n < new_parent->n)
			new_parent->left = last;
		else
			new_parent->right = last;
	}
	else
	{
		if (!last->parent)
	...
	while (last && last->parent)
	{
		...
		== -2 || binary_tree_balance(last->parent) == 2))
		{
			if (last->parent->left && last->parent->left != last)
				...
				flag = binary_tree_rotation(tree, last->parent, 1);
			else
				flag = binary_tree_rotation(tree, last->parent, 0);
		}
		if (flag)
			break;
		if (binary_tree_balance(last) == 1 && binary_tree_balance(last->parent) == 2)
			flag = binary_tree_rotation(tree, last->parent, 1);
		else if (binary_tree_balance(last) == -1 && binary_tree_balance(last->parent) == -2)
			flag = binary_tree_rotation(tree, last->parent, 0);
		else if (binary_tree_balance(last) == -1 &&
			binary_tree_balance(last->parent) == 2)
		{
			last = binary_tree_rotation(tree, last, 0);
			flag = binary_tree_rotation(tree, last->parent, 1);
		}
		else if (binary_tree_balance(last) == 1 &&
			binary_tree_balance(last->parent) == -2)
		{
			last = binary_tree_rotation(tree, last, 1);
			flag = binary_tree_rotation(tree, last->parent, 0);
		}
